...
for subwave_idx in range(num_subwaves):
    for metric_idx in range(num_metrics):
        ...
        correlations_cd[subwave_idx, metric_idx] = np.corrcoef(metric_values, data['Cd'])[0, 1]
        correlations_chl[subwave_idx, metric_idx] = np.corrcoef(metric_values, data['Chl'])[0, 1]
...
# 重新绘制Chl相关性热图，并添加格子
bx = sns.heatmap(correlations_chl, cmap=sns.color_palette("vlag", as_cmap=True), vmin=-1, vmax=1, ax=axes[0],
            xticklabels=metric_names, yticklabels=subwavelet_names, cbar_kws={'label': 'Correlation'},
...
# 重新绘制Cd相关性热图，并添加格子
ax = sns.heatmap(correlations_cd, cmap=sns.color_palette("vlag", as_cmap=True), vmin=-1, vmax=1, ax=axes[1],
            xticklabels=metric_names, yticklabels=subwavelet_names, cbar_kws={'label': 'Correlation'},
